...
func (r *Receiver) Run(ctx context.Context, tcfg protocol.TransferConfig, eventCh chan<- protocol.TaskEvent, taskID string) error {
	...
	for !completeReceived {
		...
		case <-deadline:
			...
			fmt.Printf("  [Receiver] COMPLETE 超时: received=%d total=%d complete=%v\n", received, totalSize, complete)
			if complete && received >= totalSize {
				fmt.Printf("  [Receiver] 数据已完整，跳过等待 COMPLETE\n")
				completeReceived = true
			} else {
				fmt.Printf("  [Receiver] 数据不完整，关闭连接\n")
				for _, c := range conns {
					c.Close()
				}
				wg.Wait()
