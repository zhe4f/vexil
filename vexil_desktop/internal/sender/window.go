...
type Window struct {
	...
	ackedOffset  atomic.Int64 // 新增：已确认的累计偏移
	started      bool
...
// UpdateAcked 根据新的累计确认偏移量释放窗口空间
func (w *Window) UpdateAcked(newAcked int64) {
	old := w.ackedOffset.Load()
	if newAcked > old {
		delta := newAcked - old
		w.inFlight.Add(-delta)
		if w.inFlight.Load() < 0 {
			w.inFlight.Store(0)
		}
		w.ackedOffset.Store(newAcked)
	}
	w.Notify()
}
